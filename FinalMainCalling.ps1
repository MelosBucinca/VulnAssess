...
while ($true) {
    ...
    if ($choice -eq "1") {
        ...
        # Open a new PowerShell process with admin privileges and execute the same script
        Start-Process powershell -Verb RunAs -ArgumentList "-File `"$($MyInvocation.MyCommand.Path)`", 1"
    ...
    elseif ($choice -eq "2") {
        ...
        # Open a new PowerShell process with admin privileges and execute the same script
        Start-Process powershell -Verb RunAs -ArgumentList "-File `"$($MyInvocation.MyCommand.Path)`", 2"
